...
The repo had a contract gap. AGENTS/CLAUDE/GEMINI named a "repo contract" that wasn't enforced anywhere, `skills_index.json` could silently drift from `skills/`, doc files could lose their declared sections without anyone noticing, and `cognitive_deadlift.egg-info/` style artifacts could get tracked. The Makefile also didn't expose the targets the user contract names like `prod-gate`, `docs-check`, `skills-check`, and `slop-scan`.
...
- `scripts/validate_repo.py` only checked file presence and the Claude manifest, with no index match, no doc contract, and no artifact policing.
...
- `ARCHITECTURE.md` was a one-line redirect to `docs/architecture.md`, so two files were claiming the same job, which is exactly the duplication `repo-audit.md` is supposed to catch.
...
- The slop scanner must allow docs to *name* banned phrases as examples, because listing what to avoid is part of the contract.
...
- **Make the slop scanner ignore filenames named `*-banned-list.md`.** Rejected as a brittle naming convention. The right behavior is to ignore code spans and fenced blocks, which is how every linter handles this.
...
- **Delete `docs/architecture.md` and consolidate into `ARCHITECTURE.md`.** Rejected, because the two files do different jobs. `ARCHITECTURE.md` owns top-level structure, `docs/architecture.md` owns deeper rationale and alternatives, and both have audiences.

## Chosen Approach

- Expand `validate_repo.py` with three new checks named `skills_index`, `doc_contract`, and `generated_artifacts`. Keep failures as findings printed to stderr instead of bailing on the first one.
...
- Skill grading is heuristic and gameable by line and bullet counts.
